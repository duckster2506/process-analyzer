...
class AnalysisRecordTest extends TestCase
{
    ...
    public function testCanCopyWithoutRelation(): void
    {
        // Create instance and start recording
        $original = AnalysisRecord::open("Original")->start();
        $original->setPreSnapshot(Analyzer::takeSnapshot());
        $original->setPostSnapshot(Analyzer::takeSnapshot());
        ...
        $this->assertNotEquals($original->getUID(), $copy->getUID());
        // Everything else will be exactly the same
        $this->assertSame($original->getName(), $copy->getName());
        $this->assertSame($original->getStartTime(), $copy->getStartTime());
        $this->assertSame($original->getEndTime(), $copy->getEndTime());
        $this->assertSame($original->getStartMem(), $copy->getStartMem());
        $this->assertSame($original->getEndMem(), $copy->getEndMem());
        $this->assertSame($original->isStarted(), $copy->isStarted());
        $this->assertSame($original->isStopped(), $copy->isStopped());
        $this->assertSame($original->isShared(), $copy->isShared());
        $this->assertSame($original->getPreSnapshot(), $copy->getPreSnapshot());
        $this->assertSame($original->getPostSnapshot(), $copy->getPostSnapshot());
    }

    public function testCanCopyWithRelation(): void
    {
        // Create instance and start recording
        $original = AnalysisRecord::open("Original")->start();
        $original->setPreSnapshot(Analyzer::takeSnapshot());
        ...
        // UID and relation will be renewed
        $this->assertNotEquals($original->getUID(), $copy->getUID());
        $this->assertNotSame($original->getRelations(), $copy->getRelations());
        // But it's size is equal
        $this->assertEquals(count($original->getRelations()), count($copy->getRelations()));
        // Everything else will be exactly the same
        $this->assertSame($original->getName(), $copy->getName());
        $this->assertSame($original->getStartTime(), $copy->getStartTime());
        $this->assertSame($original->getEndTime(), $copy->getEndTime());
        ...
        for ($i = 0; $i < 2; $i++) {
            ...
            $this->assertNotSame($original->getRelations()[$i], $copy->getRelations()[$i]);
            // Relation type is equal
            $this->assertEquals($original->getRelations()[$i]->isIntersect(), $copy->getRelations()[$i]->isIntersect());
            // Check owner or target
            if ($original->getRelations()[$i]->getOwner() === $original) {
                // If $original is an owner in old relation, so will $copy
                $this->assertSame($copy, $copy->getRelations()[$i]->getOwner());
            } else {
                // Else, so will $copy
                $this->assertSame($copy, $copy->getRelations()[$i]->getTarget());
            }
            $this->assertSame($copy, $copy->getRelations()[$i]->getOwner());
            // Target is the same
            $this->assertSame($original->getRelations()[$i]->getTarget(), $copy->getRelations()[$i]->getTarget());
        }
    }
